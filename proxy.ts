...
const isPublicRoute = createRouteMatcher([
  ...
  "/terms-of-service",
  "/success",
  // Clerk impersonation and other internal routes
...
export default clerkMiddleware(async (auth, req) => {
  ...
  // Handle redirects for common typos/misspellings
  const url = req.nextUrl;

  // Redirect /signin to /sign-in
  if (url.pathname === "/signin" || url.pathname.startsWith("/signin/")) {
    const newUrl = url.clone();
    newUrl.pathname = "/sign-in" + url.pathname.replace("/signin", "");
  ...
  if (url.pathname === "/signup" || url.pathname.startsWith("/signup/")) {
    const newUrl = url.clone();
    newUrl.pathname = "/sign-up" + url.pathname.replace("/signup", "");
    return NextResponse.redirect(newUrl);
  }

  // Allow public routes without authentication
  if (isPublicRoute(req)) {
    console.log("[PROXY] Public route access:", url.pathname);
  ...
  if (
    ...
    if (isImpersonating) {
      console.log(
        `[PROXY] 🚨 IMPERSONATION ACTIVE: User ${actor.sub} is impersonating user ${userId}`,
      );
      console.log(
        `[PROXY] Impersonated session ID: ${sessionId}`,
      );
    }
    ...
    if (userRole === "student") {
      ...
      if (!hasRequiredInfo) {
        console.log(
          "[PROXY] Student missing required info, redirecting to onboarding",
        );
        const onboardingUrl = new URL("/onboarding", req.url);
        return NextResponse.redirect(onboardingUrl);
      }
      ...
      if (isFacultyDashboard(req) || isFacultyOnlyRoute(req)) {
        console.log(
          "[PROXY] Student trying to access faculty route, redirecting",
        );
      ...
      if (!hasRequiredInfo) {
        console.log(
          "[PROXY] Faculty missing required info, redirecting to onboarding",
        );
        const onboardingUrl = new URL("/onboarding", req.url);
        return NextResponse.redirect(onboardingUrl);
      }
      ...
      if (isStudentDashboard(req) || isStudentOnlyRoute(req)) {
        console.log(
          "[PROXY] Faculty trying to access student route, redirecting",
        );
