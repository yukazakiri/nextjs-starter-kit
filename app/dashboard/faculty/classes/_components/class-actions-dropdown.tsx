...
import { Archive, Copy, Edit, FileDown, Loader2, MoreVertical, Settings, Trash2, Users } from "lucide-react";
...
export function ClassActionsDropdown({ classItem, onActionComplete }: ClassActionsDropdownProps) {
    ...
    const { subjectCode, subjectName, section } = LaravelApiHelpers.formatClassInfoForClassData(classItem);

    const handleArchive = async () => {
        setIsProcessing(true);
        try {
            const response = await fetch(`/api/faculty-classes/${classItem.id}/archive`, {
                method: "PATCH",
                headers: {
            ...
            toast.success(`Class ${subjectCode} archived successfully`);
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            toast.success(`Class ${subjectCode} deleted successfully`);
    ...
    const handleExport = async (type: "roster" | "grades" | "attendance") => {
        try {
            const response = await fetch(`/api/faculty-classes/${classItem.id}/export?type=${type}`, {
                method: "GET",
            });
            ...
            a.href = url;
            a.download = `${subjectCode}_${section}_${type}_${new Date().toISOString().split("T")[0]}.csv`;
    ...
    return (
        <>
                ...
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            ...
                            ? The class will be hidden from your active classes but can be restored later.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
            ...
            {/* Edit Class Modal */}
            <EditClassModal
                classItem={classItem as any} // Temporary cast until EditClassModal is updated
                open={showEditModal}
            ...
            <DuplicateClassModal
                classItem={classItem as any} // Temporary cast until DuplicateClassModal is updated
            ...
            <ClassSettingsModal
                ...
                onSettingsUpdated={onActionComplete}
            />
        </>
    );
}
